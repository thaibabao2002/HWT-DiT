...
class  Trainer:
    ...
    def _train_iter(self, data, step, pbar, scaler):
        self.model.train()
        # prepare input

        images, style_ref, laplace_ref, content_ref, img_hw, aspect_ratio, wid = data['img'].to(self.device), \
            data['style'].to(self.device), \
            data['laplace'].to(self.device), \
            data['content'].to(self.device), \
            data['img_hw'].to(self.device), \
            data['aspect_ratio'].to(self.device), \
    ...
    def _finetune_iter(self, data, step, pbar):
        ...
        x_start, predicted_noise, high_nce_emb, low_nce_emb = self.diffusion.train_ddim(self.model, x_t, style_ref,
                                                                                        laplace_ref,
                                                                                        content_ref, t,
    ...
    def _valid_iter(self, epoch):
        ...
        # prepare input
        images, style_ref, laplace_ref, content_ref, img_hw, aspect_ratio = test_data['img'].to(self.device), \
            ...
            for step, data in enumerate(pbar):
                ...
                if self.ocr_model is not None:
                    ...
                    if self.valid_data_loader is not None:
                        ...
                        self._train_iter(data, total_step, pbar, scaler)
    ...
    def _save_checkpoint(self, epoch):
        torch.save(self.model.state_dict(), os.path.join(self.save_model_dir, str(epoch) + '-' + "ckpt.pt"))
