...
def calculate_hwd_given_paths(parent_real_path, parent_fake_path, device='cpu'):
    list_fd = os.listdir(parent_real_path)

    lst_rs = []
    for fd_name in tqdm(list_fd, desc="Compute hwd:"):
        real_path = os.path.join(parent_real_path, fd_name)
        fake_path = os.path.join(parent_fake_path, fd_name)
        real_dataset = FolderDataset(real_path, extension='png')
        fake_dataset = FolderDataset(fake_path, extension='png')
        score = HWDScore(device=device)
        result = score(real_dataset, fake_dataset, verbose=False)
        lst_rs.append(result)
    return sum(lst_rs)/len(lst_rs)